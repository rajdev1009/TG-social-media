...
async function initPostgres() {
  ...
  console.log('✅ Postgres (Neon) schema ensured');
}
...
// ---------------- photo upload (avatar) -> archive to private channel ----------------

bot.on('photo', async (ctx) => {
  const tgId = ctx.from.id;
  const session = getSession(tgId);
  if (session?.flow !== 'set_avatar') return; // ignore stray photos elsewhere
...
// ---------------- voice / audio -> DM support (archived + routed via file_id) ----------------

async function handleDmMedia(ctx, mediaType, fileObj) {
  const tgId = ctx.from.id;
  const session = getSession(tgId);
  if (session?.flow !== 'sending_dm' || !session.targetId) {
    return ctx.reply('🎙 To send a voice/audio message, open a profile via 🔀 Explore and tap 💬 Message first.');
  }
  ...
  try {
    ...
    if (mediaType === 'voice') {
      await bot.telegram.sendVoice(target.telegram_id, fileId, { caption, parse_mode: 'Markdown' });
    } else {
      await bot.telegram.sendAudio(target.telegram_id, fileId, { caption, parse_mode: 'Markdown' });
    }
  } catch (e) { /* user may have blocked the bot */ }
  ...
  return ctx.reply(`✅ ${mediaType === 'voice' ? 'Voice note' : 'Audio'} sent!`, mainMenu(isAdminId(tgId)));
}
...
bot.action(/^msg:(\d+)$/, async (ctx) => {
  ...
  return ctx.reply('✍️ Send text, a voice note, or an audio file — it will be delivered anonymously (your public username shown, real identity protected):', cancelInline());
});
